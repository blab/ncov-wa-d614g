...
lreg_ct_uwmed = glm(avg_ct ~ clade + sex + week + severe + bin_age + site + immuncompromised_cancer + primers, data = df)
...
```{r}
lreg_summary_uwmed <- tibble("Variable" = c("Intercept", "614G", "Male", "Age", "Week", "Immuncomp. or\nactive cancer", "Hospitalized", "Critical care\nor deceased", "Egene, RdRp primers", "ORF1ab primers"), 
                       "Coeff. est." = c("15.87", "-1.15", "1.13", "0.013", "0.34",  "-0.10", "0.70", "-0.46", "2.96", "-0.35"), 
                       "Std. Error" = c("1.38", "0.53", "0.47", "0.013", "0.26", "0.76", "0.72", "0.99", "3.07", "0.66"),
                       "p-value" = c("<2e-16***", "0.033*", "0.017*", "0.30", "0.19", "0.89", "0.33", "0.65", "0.34", "0.59"))


table_uwmed <- ggtexttable(lreg_summary_uwmed, rows = NULL, 
                     theme = ttheme("lBlack", padding = unit(c(4, 3), "mm"), colnames.style = colnames_style(fill = "white", hjust=0, x =0.05, size=10), 
                                    tbody.style = tbody_style(fill = "white", hjust=0, x =0.05, size=10))) 
